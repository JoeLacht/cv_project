...
# --- Секция обучения модели (леса) ---
st.subheader("🌲 Обучение модели (Леса)")
with st.expander("Посмотреть детали обучения"):
    st.markdown("""
    Модель обучалась суммарно 50 эпох.  
    Итоговые метрики:
    
    - Val Accuracy: 0.833 — высокая точность на валидации  
    - Val IoU: 0.777 — уверенное качество сегментации
    
    🔎 Итог: модель показала отличные результаты. Можно использовать на практике, при необходимости дообучив на новых данных.
    """)

        # График обучения
    if os.path.exists("images/metrics.png"):
        st.image("images/metrics.png", caption="График обучения модели", use_container_width=True)
    else:
        st.info("График обучения пока отсутствует. Поместите файл 'images/metrics.png' в папку проекта.")

    # --- Коллаж с тестами ---
    if os.path.exists("images/sample.png"):
        st.image("images/sample.png", caption="Коллаж тестов", use_container_width=True)
    else:
        st.info("Коллаж тестов отсутствует. Поместите файл 'images/sample.png' в папку images.")